...
internal class ShuffledTableEncryptionAlgoTest {
  ...
  fun word_size_is_computed_correctly(base: Int, expectedWordSize: Int) {
    ...
    assertEquals(expectedWordSize, ShuffledTableEncryptionAlgo(baseSystem, RandomMock()).wordSize)
  }

  private class RandomMock : Random() {
    private var i = 0
    override fun nextBits(bitCount: Int): Int = throw RuntimeException()
    override fun nextInt(until: Int): Int = i++.coerceAtMost(until-1)
  }
  ...
  private fun createAlgo(): ShuffledTableEncryptionAlgo {
    return ShuffledTableEncryptionAlgo(createBaseSystemMock(), prng = RandomMock())
  }
  ...
  fun encrypt_is_implemented_correctly_for_word_size_1() {
    ...
    val algo = ShuffledTableEncryptionAlgo(baseSystem, RandomMock(), wordSize = 1)
  ...
  fun encrypt_is_implemented_correctly_for_word_size_2() {
    val baseSystem = mock(BaseSystem::class.java)
    val algo = ShuffledTableEncryptionAlgo(baseSystem, RandomMock(), wordSize = 2)
