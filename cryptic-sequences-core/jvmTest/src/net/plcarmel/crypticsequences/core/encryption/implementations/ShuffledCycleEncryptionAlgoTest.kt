...
internal class ShuffledCycleEncryptionAlgoTest {
  ...
  fun word_size_is_computed_correctly(base: Int, expectedDimensions: Int) {
    val baseSystem = mock(BaseSystem::class.java)
    `when`(baseSystem.base).thenReturn(base)
    assertEquals(expectedDimensions, computeDefaultTableDimensions(baseSystem))
  }
